...
use super::error::{CredentialError, UserError};
...
/// without a DB. Returns `UserError::Validation` on bad input and
/// `UserError::Db` (wrapping a `Custom` `DbErr`) on hashing failure — the
/// HTTP layer maps both as it would for a real DB error.
pub(crate) fn prepare_new_user(
    input: CreateUserInput,
    org_id: Uuid,
    password: Option<&str>,
) -> Result<entity::ActiveModel, UserError> {
    input.validate()?;
...
mod tests {
    ...
    fn prepare_new_user_rejects_empty_name() {
        ...
        match err {
            UserError::Validation(v) => assert!(v.field_errors().contains_key("name")),
    ...
    async fn register_with_password_surfaces_executor_error_as_db_variant() {
        ...
        // the resulting `UserError::Db` carries the synthetic `DbErr::Custom`
        // and the wire string is the opaque "database error".
        let err = svc
            .register_with_password("ada@example.com", "ada", "hunter2", Uuid::now_v7())
            .await
            .expect_err("no executor ⇒ UserError::Db");
        assert!(matches!(err, UserError::Db(_)));
