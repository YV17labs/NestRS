...
/// `oauth_resource`). Dual-path like every `nest-rs-*` config: `NESTRS_OAUTH_RESOURCE__*` env
/// vars over the base pinned in
/// [`OAuthResourceModule::for_root`](crate::OAuthResourceModule::for_root),
/// composing per field.
#[config(namespace = "oauth_resource")]
...
pub struct OAuthResourceConfig {
    ...
    /// `https://api.example.com/mcp`. Read from `NESTRS_OAUTH_RESOURCE__RESOURCE`;
    /// **required** — the module fails boot without it.
    pub resource: Option<String>,
    /// Issuer identifiers of the authorization servers that mint tokens for
    /// this resource. Read from `NESTRS_OAUTH_RESOURCE__AUTHORIZATION_SERVERS`;
    ...
    pub fn into_metadata(self) -> Result<ProtectedResourceMetadata, AuthError> {
        ...
        if resource.is_empty() {
            return Err(AuthError::Failed(format!(
                ...
                var_name(Self::NAMESPACE, "RESOURCE"),
            )));
        }
        ...
        if authorization_servers.is_empty() {
            return Err(AuthError::Failed(format!(
                ...
                var_name(Self::NAMESPACE, "AUTHORIZATION_SERVERS"),
            )));
        }
