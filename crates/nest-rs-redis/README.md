...
Redis as one integration home: `RedisModule` opens the one shared connection pool (`NESTRS_REDIS__*`), and one binding per port sits beside it — `RedisQueueModule` (the portable `dyn JobProducer`, via oxana), `RedisWorkerModule` (the `RedisWorker` consumer transport), and `RedisThrottlerModule` (the rate-limit store shared across replicas). The user-facing storage is Redis; oxana is an implementation detail.
