...
Redis as one integration home for the framework's two cross-process needs: the queue (via apalis-redis) — QueueConnection producer, QueueWorker consumer transport, #[processor] re-export — and RedisThrottler, the rate-limit store shared across replicas. The user-facing storage is Redis; apalis is an implementation detail.
