//! C2: `QueueModule::for_root` must bind the **portable** producer name too.
//!
//! `/queue/producing-jobs/` tells a feature to inject either
//! `Arc<QueueConnection>` (concrete) or `Arc<dyn JobProducer>` (portable), and
...
use nest_rs_redis::{QueueConfig, QueueConnection, QueueModule};
...
fn dev_container_queue() -> QueueConfig {
    QueueConfig {
        url: redis_url(),
        ..Default::default()
    }
...
#[module(imports = [QueueModule::for_root(dev_container_queue())])]
...
async fn for_root_binds_both_the_concrete_and_the_portable_producer_name() {
    ...
    assert!(
        app.container().get::<QueueConnection>().is_some(),
