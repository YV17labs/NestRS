...
//!
//! Three rules are pinned here because each one is a security or DX contract the
...
//! decision outside a guard* — silence is not a posture); and the two wrong-shape
//! cases pin the *one decorator, one item shape* rule, each naming the sibling
//! that belongs on the shape the developer reached for.
