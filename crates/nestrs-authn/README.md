...
- The OAuth Authorization Code client (`OAuth2Client`) is *configuration*,
  ...
  `features::oauth/core/strategies/` (the OAuth strategy needs DB-backed
  state which is feature-side concern).

A community impl is named `nestrs-authn-<scheme>` — e.g.
`nestrs-authn-opaque-tokens` (token introspection against a remote
authorization server), `nestrs-authn-mtls` (client certificate),
`nestrs-authn-passkey`. Each ships a `Strategy` implementor (an
