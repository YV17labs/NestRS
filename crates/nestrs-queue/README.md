...
apalis-redis), shipped as [`nestrs-redis`](../nestrs-redis/). A third-party
...
`nestrs-queue-macros`) so the call site keeps writing `use
nestrs_queue::processor;` regardless of which storage integration is wired
in.

## The link-time registry

The `#[processor]` macro (in `nestrs-queue-macros`, re-exported by this
...
[`ReachableProviders`](::nestrs_core::ReachableProviders) so methods on
...
1. **Depend on `nestrs-queue`** (this crate) — get the abstractions and the
   `#[processor]` macro through the same import root. Application code in
   your downstream users keeps writing `nestrs_queue::processor`
   unchanged.
...
The Redis implementation in [`crates/nestrs-redis/`](../nestrs-redis/) is
