...
The **Redis-backed integration** of [`nestrs-queue`](../nestrs-queue/) — the
...
you write your own `nestrs-<storage>` crate against `nestrs-queue` — the
...
`JobProducer` types come from [`nestrs-queue`](../nestrs-queue/) — this
...
`v` is `nestrs_queue::WIRE_FORMAT_VERSION`. The `#[processor]`-generated
...
with a `warn` on target `nestrs::queue`, so jobs already in flight at
...
[`crates/nestrs-queue/README.md`](../nestrs-queue/README.md) for the
