...
impl AppOAuthService {
    ...
    pub fn authorize(&self, provider: &str) -> Option<Result<AuthorizationRedirect, AuthError>> {
...
pub(crate) fn issue_with_jwt(
    ...
    let claims = Claims {
        ...
        org_id,
        roles: roles.clone(),
        scopes: crate::app_authz::constants::all(),
        exp: jwt_svc.expiry(),
    };
...
pub(crate) fn grant_client_credentials_with_jwt(
    ...
    if grant_type != "client_credentials" {
        tracing::warn!(target: "features::app_oauth", grant_type, "unsupported grant type");
...
mod tests {
    ...
    fn oauth_service(ttl: Duration) -> AppOAuthService {
        ...
        AppOAuthService::new(jwt_svc, providers, users_service_disconnected(), config)
    }
