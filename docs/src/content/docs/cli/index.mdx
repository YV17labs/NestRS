...
Doctor asks whether the machine is right; lint asks whether the code is. It
...
That file was named for a *slot* — "who acts", "what we pass around" — rather
...
Everything short of that passes. The word may come from the folder rather than
the file (`throttler/store.rs` holding `RedisThrottler`), an inflection is the
same word (`scope.rs` holding `Scoped`), an abbreviation is too (`context.rs`
holding `Ctx`), and a file whose principal export is a function is a namespace
that owes no pairing at all (`queue/consume.rs` exporting `consume::attempt`).
...
The framework holds itself to it through the same code, from its own
conformance suite — a rule shipped and not met is the failure that matters.
