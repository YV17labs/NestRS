...
# name AppAuthnGuard/AppAuthzGuard, so they would not compile without it.
nestrs g resource posts
...
# The app's authn/authz adapter — Claims, AppAuthnGuard, AppAbility, AppAuthzGuard.
# Run it *before* the first `g resource`, or not at all: it refuses when
# `crates/features/src/app_authz/` already exists.
nestrs g auth
...
under `crates/features/src/app_authz/` when the workspace has a policy to enforce,
...
`g resource` emits the `#[crud]` + `#[use_guards(AppAuthnGuard, AppAuthzGuard)]` form,
...
through: `crates/features/src/app_authz/graphql/` plus the `AppAuthzGraphqlModule`
