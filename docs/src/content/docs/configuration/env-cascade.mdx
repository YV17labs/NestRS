...
- **`ConfigModule::for_root()`** parses it during the collect phase,
  before any factory runs. This is the expected path.
- **The default `EnvSource`** parses it on its first `get`. A
  [custom `ConfigSource`](/configuration/sources/) never does — it
