...
The one pool every SeaORM binding shares, opened by `SeaOrmModule::for_root`.
...
The one Redis connection every Redis binding shares — the queue producer, the
...
The consumer binding's own settings, resolved by `RedisWorkerModule::for_root`.
