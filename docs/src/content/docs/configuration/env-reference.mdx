...
| `TRUSTED_PROXIES` | comma-separated reverse-proxy IPs whose `X-Forwarded-For` / `X-Real-IP` are believed. Empty (the default) ⇒ neither header is read and every caller is its transport peer. Shared by [`ClientIp`](/http/extractors/) and the [throttler](/rate-limiting/); an unparseable entry aborts the boot |
