...
| `CONTENT_SECURITY_POLICY` · `CROSS_ORIGIN_EMBEDDER_POLICY` · `PERMISSIONS_POLICY` | settable, off by default — each governs a *document* whose sources the framework cannot speak for; see [HTTP configuration](/http/configuration/) |
...
`OAuthClientConfig` shares this namespace for the login flow's provider endpoints —
...
`OAuthResourceModule` shares the namespace too — it is what turns the app
...
it rather than leaving the app unable to say which tokens are its own. (A token
minted for *another named* audience is refused with or without the module — that
is RFC 7519 §4.1.3. What the module adds is that a token carrying no `aud` at
all is refused too.)
...
`graphql`, `openapi`, `ws`, `throttler`, `health`, and the `auth` app's `issuer` each read
