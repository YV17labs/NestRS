...
A `#[config]` struct derives `validator::Validate` and carries `#[validate]`
rules, and the crate re-exports neither the trait nor its derive — so the
version has to match the one `nest-rs-config` compiles against, or the derive
implements a *different* copy of the trait and the error reads as though you
wrote the impl wrong:
...
`nestrs new` writes that pin into `[workspace.dependencies]` for you.
