...
```rust title="crates/features/src/posts/config.rs"
...
- [The .env cascade](/configuration/env-cascade/) — which files load, in
  what order, and how the active `Environment` selects them.

### All options

- [Alternative sources](/configuration/sources/) — Vault, a K8s ConfigMap
  or AWS Parameter Store behind `ConfigSource`.
- [Overriding in tests](/configuration/testing/) — seed pinned values,
  isolate the environment, keep both suites hermetic.
- [Advanced: hand-written `from_env`](/configuration/advanced/) — custom
  validation and nested config, when the derive is not enough.
- [Env-var reference](/configuration/env-reference/) — every `NESTRS_*`
  variable the framework reads, by namespace.
...
- [Modules](/fundamentals/modules/) — where `for_root(...)` sits in a
  composition root.
- [HTTP configuration](/http/configuration/) — the largest config struct the
  framework ships, dual-path like every other.
- [CLI](/cli/) — `nestrs run` loads the cascade before it starts an app.
