...
clients accepted by `apps/publish-auth`):
...
- **`#[config(namespace = "issuer")]`** binds the struct to the
  `NESTRS_ISSUER__*` scheme. The macro wires the `Namespaced` trait — no
...
- **`impl Validate`** runs after the load. A failure aborts boot naming
  the field. For per-field rules, derive `Validate` and use
  `#[validate(...)]` attributes instead of implementing it by hand.

`env.parse("DEFAULT_ORG_ID")?` reads + parses into the field's type;
unset returns `None`, set-but-unparseable returns `Err` naming the
...
```rust title="apps/publish-auth/src/module.rs"
use nest_rs_config::ConfigModule;
...
A consumer injects `Arc<IssuerConfig>` like any provider — the type is
the token.
...
```bash frame="terminal"
$ NESTRS_ISSUER__DEFAULT_ORG_ID=not-a-uuid just dev publish-auth
...
```bash frame="terminal"
$ just dev publish-auth   # no clients configured
