...
[dependencies]
nest-rs = { version = "3.1", features = ["config"] }
```
...
A module that also carries a declaration with **no env twin** — an MCP
endpoint's name and version — takes an `Options` struct instead, so the two
still travel in one value:
