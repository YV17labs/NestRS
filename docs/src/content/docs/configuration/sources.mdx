...
- **A test is hermetic when it does not call `Environment::init`** (or
  `load_cascade`, its explicit form). `with_source` additionally decides where
  the values come from — useful, but a different question from whether the
