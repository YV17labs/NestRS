...
`#[authorize(Action, Entity)]` (gate + automatic response masking),
`#[public]`, or — for a bound mutation — an `Authorized<E, A>` parameter
alone (next section); an operation with none does not compile.

Same override rule — the GraphQL `create_user` and `user` you wrote take
over; `users`, `update_user`, `delete_user` are generated.

## Signature-only mutations

A **bound mutation** acts on one existing row the caller already named by
id — `publish`, `archive`, `confirm`. Instead of repeating the service in
an attribute, declare the subject as an **`Authorized<E, A>` parameter** and
let the type be the policy. No `#[authorize]` attribute:
...
impl ArtworksResolver {
    ...
    async fn publish_artwork(
        &self,
        artwork: Authorized<ArtworkEntity, Update>,
    ) -> Result<Artwork> {
...
From that one parameter, `#[crud]` synthesises everything the explicit
`#[authorize(Update, bind = ArtworksService)]` form would, reading each
piece off what is already declared:

- the **action** (`Update`) and **entity** (`ArtworkEntity`) — from the
  parameter type;
- the **service** — from `#[crud(service = svc)]`, so the row binds through
  `bind_required_with(&*self.svc, ctx, &id)` (the service *instance*, no
  type retyped);
- a by-id `id: String` argument in the subject's place (uniform with every
  other by-id operation);
- the class gate `authorize::<Update, ArtworkEntity>` before the load and
  the response mask after it.
...
is sealed, mintable only by the binding seams, so a hand-written mutation
can neither reach a row the caller could not load nor act under an action
it was not granted.

<Aside type="note">
Signature-only is GraphQL + `#[crud]` only: it needs the service field from
`#[crud(service = …)]`. Outside `#[crud]`, name the service explicitly —
`#[authorize(Update, bind = ArtworksService)]` (container-resolved) still
works and is never forced out.
</Aside>
