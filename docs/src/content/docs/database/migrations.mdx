...
```rust title="crates/migrations/src/m20260526_000000_create_org.rs (from the demo)"
...
```rust title="crates/migrations/src/m20260526_000001_create_user.rs (from the demo)"
...
```rust title="crates/migrations/src/migrator.rs (from the demo)"
use sea_orm_migration::prelude::*;
...
outside the DI container, which resolves `NESTRS_SEAORM__*` through the same
`.env` cascade the apps use. Reaching for `std::env::var("NESTRS_SEAORM__URL")`
