...
- [Transactions](/database/transactions/) — auto-commit, conflict
  observation, the retry primitive.
- [Row-level filtering](/security/authorization/row-level-filtering/) —
  the ability condition `Repo::scoped` joins in.
- [Writing a driver](/database/writing-a-driver/) — implementing
  `Executor` for a non-SeaORM store on the same seam.
