...
```rust title="nest_rs::filters::Filter · nest_rs::exception_filters::ExceptionFilter"
#[async_trait]
...
```rust title="apps/api/src/filter.rs"
...
```rust title="the three scopes — global, controller, route"
// Global, in main.rs
App::builder()
...
```rust title="apps/api/src/exception_filter.rs"
use nest_rs::core::{Layer, injectable};
...
:::caution[A global `ExceptionFilter` covers the route table, not the edge]
"Global" means a different thing for the two families. A global `Filter`
attaches a wrap at the transport edge, so it maps errors from paths no
route matched. A global `ExceptionFilter` is read **per route** by the
`#[routes]` composer, so a 404, a self-mounted surface such as `/graphql`
or `/mcp`, or a WS upgrade never reaches one — even when the error raised
there is exactly the type it claims. Use a `Filter` for those.
:::
