...
- **MCP** — an `McpOperationGuard` (e.g. `McpAbilityBridge`), the exact twin:
  ...
  binds ordinary guards per host and per operation; whatever the endpoint guard
  reports it already ran is dropped from that chain, so a guard runs once and
  one the endpoint never ran still runs.
...
entry used to compile and pass everything. Three marker traits close that:
...
| `#[controller]` / `#[routes]`, `#[gateway]` struct | *nothing* | `check_http` |
...
HTTP takes no marker on purpose: `check_http` is the trait's base entry —
the one method not behind a feature — so every `Guard` has it, and a
bound there could never fail. A guard bound **globally** takes no marker
either: `use_guards_global` runs on the underlying HTTP request, which is
also why a global `ThrottlerGuard` does rate-limit `/graphql` and `/mcp`.
