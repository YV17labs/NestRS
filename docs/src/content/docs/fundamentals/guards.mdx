...
```rust
#[resolver]
#[use_guards(AuthzGuard)] // AbilityGuard<AppAbility> — has check_graphql
pub struct PostResolver { /* ... */ }
```

The **marker** `GraphqlAuthnGuard` is not a guard and is never bound with
`#[use_guards]`. It is a plain `#[injectable]` provider listed in
`AuthzGraphqlModule`, named by `forward_principal!(Claims,
GraphqlAuthnGuard)` as the owner of the context seed — so the seed only
runs when that provider is reachable, which is how forgetting the authz
module becomes a boot error instead of a silently unauthenticated schema.
