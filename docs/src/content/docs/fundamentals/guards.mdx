...
2. **Secure the post API** — `#[use_guards(AppAuthnGuard, AppAuthzGuard)]` on
...
```mermaid
flowchart LR
  A["① Guard trait<br/>gate or pass"] --> B["② PostsController<br/>AppAuthnGuard + AppAuthzGuard"]
...
```rust title="crates/features/src/app_authn/module.rs"
#[module(
...
```rust title="crates/features/src/app_authz/http/module.rs"
#[module(
...
- crates/features/src/
  - authn/
    - module.rs (AppAuthnModule — provides AppAuthnGuard)
    - guard.rs
  - authz/
    - module.rs (AppAuthzModule — AppAbility)
    - http/
      - module.rs (AppAuthzHttpModule — provides AppAuthzGuard)
      - guard.rs
  - posts/
    - http/
      - module.rs (PostsHttpModule — imports AppAuthzHttpModule)
      - controller.rs (#[use_guards(AppAuthnGuard, AppAuthzGuard)])
</FileTree>

```mermaid
flowchart TB
  REQ[HTTP request] --> AG[AppAuthnGuard]
  AG --> AZ[AppAuthzGuard]
  AZ --> AD{AdminGuard?}
...
`AdminGuard` on top. `AppAuthnGuard` attaches `Claims`; `AppAuthzGuard` builds
...
|-------|---------|-------------|
| **Global** | `App::builder().use_guards_global([guard::<AppAuthnGuard>()])` in `main` | Per-route shaper, post-routing (self-mount edge for gateways) |
| **Controller / Resolver / Gateway** | `#[use_guards(AppAuthnGuard, AppAuthzGuard)]` on the struct | Container, at mount |
...
```rust
#[async_trait]
impl Guard for AppAuthnGuard<MyStrategy> {
    async fn check_http(&self, req: &mut HttpRequest) -> Result<(), Denial> {
...
impl HttpGuard for AppAuthnGuard<MyStrategy> {}
...
`AppAuthzGuard` reads `Claims` left by `AppAuthnGuard` to build the ambient
`Ability`. That is why order matters:
`#[use_guards(AppAuthnGuard, AppAuthzGuard)]`, not the reverse.
...
```rust
#[get("/admin/audit")]
#[use_guards(AppAuthnGuard, RolesGuard)]
...
each guard decides what "public" means (`AppAuthnGuard` may authenticate when
a token is present but not reject anonymous callers; `AppAuthzGuard` may
...
`AppAuthnGuard` and `AppAuthzGuard` run **exactly once** per request, not twice.
...
```mermaid
flowchart TB
  MAIN["main: use_guards_global([AppAuthnGuard, AppAuthzGuard])"]
  PC["PostsController: #[use_guards(AppAuthnGuard, AppAuthzGuard)]"]
...
already ran it in band before the operation, so listing an `AppAuthnGuard`
on a resolver would bind a guard with nothing to do.

```rust
#[resolver]
#[use_guards(AppAuthzGuard)] // AbilityGuard<AppAbility> — has check_graphql
pub struct PostResolver { /* ... */ }
```

The principal reaches `ctx` through `forward_principal!(Claims)`, declared
once beside `AppAuthzGraphqlModule`. It is not a guard and binds nothing: it
...
`AppAuthzWsModule` and the guards are unreachable ⇒ boot fails). A
per-message check binds a real `Guard` beside the
`#[subscribe_message]`; it runs through `check_ws_message` — e.g.
`AppAuthzGuard` (the app's `AbilityGuard<AppAbility>` alias) denies,
fail-closed, when no ambient ability is present.

```rust
#[gateway(path = "/posts/live")]
#[use_guards(AppAuthnGuard, AppAuthzGuard)] // real guards — run once, at the upgrade
...
impl PostGateway {
    ...
    #[public]
    #[use_guards(AppAuthzGuard)] // re-checked per message via check_ws_message
...
- **Order matters** — `AppAuthnGuard` before `AppAuthzGuard`; global
  interceptors outside global guards (`DbContext` before `AppAuthzGuard`).
</Aside>
...
- [Error handling](/fundamentals/exception-filters/) — typed catch for
  handler errors.
- [Security](/security/) — `AppAuthnGuard`, `AbilityGuard`, the full chain.
