...
If `PostsController` lived in `PostsModule`, every binary that imported
...
both reach `SeaOrmDatabaseModule` (the bare binding beside `SeaOrmModule`), its
providers are built once. Dynamic
modules do **not** dedupe — each call carries its own config.

<Aside type="tip">
Env vars (`NESTRS_<NAMESPACE>__<KEY>`) fill a dynamic import's config at boot.
Pass `None` for env-only; pass a config to set defaults in code that the
environment can still override **per field** — never a lock, see
[the dual-path rule](/configuration/#the-dual-path-rule). For the imports above:
...
```sh title=".env"
NESTRS_SEAORM__URL=postgres://localhost/blog
...
`HttpModule` starts the transport. `PostsHttpModule` registers
...
- **Port vs adapter** — data in `PostsModule`, routes in `PostsHttpModule`; import the adapter at the root when you want that transport.
...
```rust title="a diamond in the import graph"
