...
feature adapters**. The app root imports **`PostHttpModule`**, not
`PostModule` — the port rides along inside the adapter's own `imports`.
...
`publish-worker` binary would import `PostModule` (or a queue adapter)
