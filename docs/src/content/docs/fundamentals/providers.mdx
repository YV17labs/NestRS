...
```rust title="three providers, three scopes"
...
`nest_rs::graphql::Scoped<T>` (from the async-graphql `&Context`), MCP
tools with `nest_rs::mcp::Scoped<T>` — the same shape as HTTP's
...
Use for throw-away per-call state (a one-shot builder — never a
correlation id, which the framework already carries). A transient must not depend on itself — cycles panic at
...
`#[async_trait]` is required on both the trait and the impl — an async method
in a `dyn`-compatible trait needs the boxed-future desugaring. It comes from
`nest_rs::core`, so this is not a manifest line.
