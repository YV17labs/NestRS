...
A `#[field_resolver]` asks the loader for one value per parent, the
loader collects every key the request needs, the framework fires one
query at the end. The user writes one method on a service; the loader
struct, the registration in the async-graphql request context, the
batch spawner that re-installs the request's ambient executor and
`Ability` around each batch — all of that lands from the macro.
...
## Per-request scope, ambient `Ability`
