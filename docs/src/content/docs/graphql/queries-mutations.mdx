...
`#[mutation]` declares one of three, and a method with none does not
compile:

- `#[public]` — deliberately ungated.
- `#[authorize(Action, Entity)]` — ability gate + automatic response
  masking.
- *(bound mutations under `#[crud]` only)* an **`Authorized<E, A>`
  parameter alone** — the subject's type *is* the posture: `#[crud]`
  reads the action + entity off it, binds the row by id, gates, and
  masks, so no attribute is written. See
  [Database / CRUD](/database/crud/#signature-only-mutations).
