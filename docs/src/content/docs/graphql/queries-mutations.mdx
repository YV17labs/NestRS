...
impl ProductsResolver {
    #[mutation]
    async fn create_product(&self, input: CreateProductInput) -> Product {
        Product {
            id: "p-001".into(),
            name: input.name,
...
resolver only translates between the wire shape and the service. See
[Database / CRUD](/database/crud/) for the service contract.
