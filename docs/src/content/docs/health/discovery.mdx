...
```
 WARN nest_rs::health: skipped indicator: provider unreachable from
      app's module tree indicator="upstream" kind=Readiness
```

`warn`, so it shows at any production filter, and once — a wiring notice
repeated on every probe is log volume, not a signal. Probing then skips in
