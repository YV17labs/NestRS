...
```rust
use nestrs_http::{HttpConfig, HttpModule, TlsConfig};
...
use nestrs_http::{CorsConfig, HttpConfig, HttpModule};
...
The value is sourced from the `nestrs-http` crate's `CARGO_PKG_VERSION` at
