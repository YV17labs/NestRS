...
`nest_rs::seaorm::ServiceError` and the OAuth `nest_rs::authn::TokenError`
...
`IntoResponse` instead — see [the table below](#framework-errors-carry-their-own-mapping).)
...
Everything above decides how an error is *shaped*. `Opaque` decides whether
...
`.opaque()?` logs the real error at `error` on `nest_rs::http` and returns a
...
Reach for it on a failure that is none of the caller's business. A validation
rejection or an authorization denial is the opposite case: those exist to be
read, and travel as themselves. The same seam reads identically on
[GraphQL](/graphql/errors/), [WebSockets](/websockets/messages/) and MCP.

## Framework errors carry their own mapping

The framework's error types carry their own HTTP mapping, so they surface
on a handler that calls a service without a single line of conversion code:
...
| `nest_rs::authn::TokenError` | OAuth token endpoint | `ResponseError` — `400` bad grant/scope, `401` invalid credentials, `500` signing/backend |
...
- [Responses](/http/responses/) — the success side, and how `#[http_code]`
  leaves a handler's `Err` status untouched.
