...
`nest_rs_seaorm::ServiceError` and the OAuth `nest_rs_authn::TokenError`
ship `ResponseError` impls, so service-layer `?` works without a
feature-level conversion. (`nest_rs_authn::AuthError` maps itself through
...
|-------|--------|------------|
| `nest_rs_seaorm::ServiceError` | Repo, CrudService, dataloader | `problem+json` — `422` validation/invalid, `409` conflict, `403` forbidden, `404` not found, `500` else (validation field errors ride as `errors`) |
| `nest_rs_authn::TokenError` | OAuth token endpoint | `ResponseError` — `400` bad grant/scope, `401` invalid credentials, `500` signing/backend |
| `nest_rs_authn::AuthError` | Strategy authentication | `IntoResponse` — `401` challenge (`500` when the identity store is unavailable) |
