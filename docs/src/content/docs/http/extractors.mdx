...
`#[input]` carries `JsonSchema` too, so a DTO behind it needs no second derive
for the OpenAPI document; a hand-derived DTO (like `CreatePost` above) does.
...
`X-Forwarded-For`) lives in the [throttler](/rate-limiting/), not here — the
