...
The bridge answers *who is calling*, and it answers it in the logs too.
Every operation runs in its own [`mcp.operation`
span](/fundamentals/correlation/), under the trace of the HTTP request it
...
Each operation declares what it is, with the same two attributes a
`#[query]` takes:
