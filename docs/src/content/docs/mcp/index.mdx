...
Below is the real `mcp` example (`apps/mcp`), running on port 3004.
...
```text frame="terminal"
2026-06-03T10:34:07.882Z  INFO nestrs::http: bound 1 MCP server on 0.0.0.0:3004
  POST /mcp → WeatherTool (1 tool)
```

Point an MCP-aware client at `http://localhost:3004/mcp`:
...
`McpAbilityBridge` (provided by `nestrs-authz` with the `mcp` feature
...
- `crates/nestrs-mcp/` — `#[mcp]`, `#[tool_router]`, `#[tool]`,
  `#[tool_handler]`.
