...
- [Authorization](/mcp/authorization/) — the bridge every tool call is gated by,
  and the posture each operation declares.
- [Endpoints](/mcp/endpoints/) — several features on one URL, and what the
  client is told the server is.
- [Resources and prompts](/mcp/resources-and-prompts/) — the two capabilities
  beside tools, and the host shape they need.
- [Configuration](/mcp/configuration/) — , the env keys, and how a
  failing operation talks to a model.
- [Guards](/fundamentals/guards/) — the trait an MCP guard implements.
