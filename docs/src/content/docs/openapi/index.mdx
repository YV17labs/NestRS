...
type. **A handler returning poem's `SSE` needs no annotation at all**: that type
serializes as `text/event-stream` and nothing else, so the document reads it off
the return type, exactly as it reads a payload schema off a `Json<T>` return.
...
#[get("/events")]
async fn events(&self, resume: Header<StreamResumeDto>) -> SSE { /* ... */ }
```
