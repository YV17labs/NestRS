...
- a shared `components.schemas`, plus a `bearerAuth` entry in
  `components.securitySchemes` and per-operation `security` on guarded routes;
- the error statuses an operation can actually produce — `400` / `404` / `429`
