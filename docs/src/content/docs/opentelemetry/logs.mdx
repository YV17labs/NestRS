...
carries.** It is declared once, in `nest_rs_core::operation_log::unit`, and read
three times: by the span, by the log record's event name (what an OTLP exporter
reports as `event.name`), and by the message a console prints. There used to be
...
diagnostic with nothing on the console to say so. `nest_rs::operation` prefixes
no other target, and a test fails the day one prefixes another.
