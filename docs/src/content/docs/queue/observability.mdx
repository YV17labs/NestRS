...
| `nest_rs::access` | One `job ran` line per attempt — identity, `outcome`, `duration_ms`. Shared with every other edge. | `info`. |
...
worker at `info` logs one `job ran` line per attempt and, on failure, one
...
`job ran` line on `nest_rs::access` — the same line every edge files for
...
dead-letters — and the `job ran` line reports `outcome="error"` (or
...
line per poll on top of the `job ran` line and any failure detail.
