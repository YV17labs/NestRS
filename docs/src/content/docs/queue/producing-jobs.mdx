...
description: How an HTTP handler enqueues — the typed queue handle, the RedisQueueConnection publisher, and the service-as-choke-point rule.
tier: basics
sidebar:
  order: 2
---
...
delegate. They do **not** hold `Arc<RedisQueueConnection>` and do **not**
know the queue name.
...
`RedisQueueConnection` and on `Arc<dyn JobProducer>` alike. It is `async`
...
<Aside>
Inject `Arc<RedisQueueConnection>` for the concrete type, or
`Arc<dyn nest_rs::queue::JobProducer>` to keep the feature portable across
backends. `RedisQueueModule::for_root` binds **both** names to the one connection,
