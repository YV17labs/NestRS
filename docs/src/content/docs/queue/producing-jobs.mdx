...
description: How an HTTP handler enqueues — the typed queue handle, the JobProducer publisher, and the service-as-choke-point rule.
tier: basics
sidebar:
  order: 2
---
...
**Plain `serde` derives, not `#[input]`** — the one place the wire-DTO
shorthand is the wrong tool. `#[input]` carries `deny_unknown_fields`, which
is right at an edge a client controls and wrong on a producer↔worker contract:
a producer one deploy ahead that adds a field would have every job it enqueues
refused by the older worker, and a decode failure aborts rather than retries,
so the job [dead-letters](/queue/retries-and-failure/) on its first attempt.
...
delegate. They do **not** hold the producer and do **not** know the queue
name.

```rust title="crates/features/src/audio/service.rs (from the demo)"
use std::sync::Arc;
...
<Aside>
`Arc<dyn nest_rs::queue::JobProducer>` is the form to inject: the feature names
no backend, so it compiles unchanged against a different driver.
`RedisQueueModule` binds **both** that name and the concrete
`nest_rs::redis::RedisQueueProducer` over the one connection `RedisModule`
opens, so the concrete type resolves too when a service genuinely needs it.
Same publisher surface either way — `push_to` is on the extension trait, not
the concrete type.
</Aside>
...
```rust title="crates/features/src/audio/dtos/transcode_dto.rs (from the demo)"
...
```rust title="crates/features/src/audio/http/controller.rs (from the demo)"
use std::sync::Arc;

use nest_rs::http::{controller, routes};
use nest_rs::http::poem::http::StatusCode;
use nest_rs::http::poem::web::Json;
use nest_rs::http::poem::{Error, Result};
