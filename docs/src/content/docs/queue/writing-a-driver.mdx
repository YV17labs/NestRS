...
description: A backend-agnostic scaffold — the shape of a queue driver for any storage with a push and a receive, durable or ephemeral, with the framework's seams filled in and the storage-specific code marked as your own.
sidebar:
  order: 2
---
...
The [`nestrs-queue`](https://github.com/NestRS/NestRS/tree/main/crates/nestrs-queue)
...
is [`nestrs-redis`](https://github.com/NestRS/NestRS/tree/main/crates/nestrs-redis)
...
`nestrs-core`, `nestrs-queue`, plus whatever your storage and runtime
...
`WIRE_FORMAT_VERSION` is exported from `nestrs-queue`; use it instead of
...
#[async_trait]
impl nestrs_queue::JobProducer for YourConnection {
    async fn push_json(&self, queue: &str, payload: serde_json::Value) -> anyhow::Result<()> {
        ...
        let envelope = serde_json::json!({
            "v": nestrs_queue::WIRE_FORMAT_VERSION,
            "payload": payload,
        });
...
This is the meat. The worker is a `nestrs_core::Transport`: the
...
```rust
use async_trait::async_trait;
use nestrs_core::{Container, ReachableProviders, Transport, injectable, inventory};
...
impl Transport for YourWorker {
        ...
        -> anyhow::Result<()>
    {
        ...
        //     is silently skipped — same property nestrs-redis relies on.
...
Two modules — same split as `nestrs-redis::{QueueModule, QueueWorkerModule}`.
A producer-only app (an API binary that enqueues but doesn't drain)
imports the first; a worker app imports both.

```rust
use nestrs_core::{ContainerBuilder, DynamicModule, Module, TransportContribution};
...
impl DynamicModule for YourQueueSetup {
    fn collect(&self, builder: ContainerBuilder) -> ContainerBuilder {
        ...
        // `Arc<dyn nestrs_queue::JobProducer>` so callers stay portable.
        builder
    }
}
