...
```rust title="crates/yourqueue/src/queue/producer.rs"
...
```rust title="crates/yourqueue/src/worker/transport.rs"
...
```rust title="the three modules — substrate, producer, consumer"
use std::sync::Arc;
...
```rust title="two binaries, one integration"
