...
This is the meat. The worker is a `nest_rs::core::Transport`: the
...
long-poll, `ReceiveMessage`), how retries and visibility timeouts wire
up. Those are storage decisions.
...
```rust
use nest_rs::queue::async_trait;
use nest_rs::core::{Container, ReachableProviders, Transport, inventory};
                ...
                loop {
                    tokio::select! {
                        ...
                        envelope = /* your_pop_from_queue(queue) */ => {
                            // (4) Take the producer's trace context off the
                            //     value, then dispatch.
                            let (envelope, inherited) =
                                nest_rs::queue::envelope::open(envelope);
                            //     The macro-emitted handler takes
                            //     the serde_json::Value and the Container.
                            //     It internally:
...
`inherited` is `None` for a legacy or foreign payload: mint a
`Correlation` there and open the job's span under it through
`nest_rs::core::operation_span!`, the way `nest-rs-redis` does. Without
that span the job's events file under nothing.
...
Two modules — same split as `nest-rs-redis::{RedisQueueModule, RedisWorkerModule}`.
...
impl DynamicModule for YourQueueSetup {
    fn collect(&self, builder: ContainerBuilder) -> ContainerBuilder {
        ...
        // `Arc<dyn nest_rs::queue::JobProducer>` so callers stay portable.
        builder
    }
}
...
impl, same `#[process(queue = <Name>Queue)]`, same method signatures.
