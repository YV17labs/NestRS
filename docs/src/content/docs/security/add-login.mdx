...
`Claims` they name and the ability policy — but *not* a login route: what
counts as a credential, where users live and what the token carries are domain
decisions. The parts underneath are in the box, so the handler stays thin:
...
pub type AppJwtStrategy = nest_rs_authn::JwtStrategy<Claims>;
...
`AbilityGuard` lives at `nest_rs_authz::http`, behind the crate's `http`
