...
A strategy implements one async method. It returns the principal on
success (`Ok`), or an `AuthError` describing why it could not (`Err`).
A strategy never issues a transport response itself — a redirect-style
flow (OAuth `/authorize`) is a plain handler, so authentication stays a
pure request → principal mapping.
