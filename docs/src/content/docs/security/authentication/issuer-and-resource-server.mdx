...
authorization server to ask. `ProtectedResourceModule` answers that,
...
without it, naming the variable. That is the confused-deputy defence:
a resource server that does not pin `aud` accepts any token its issuer
signed, including one a user granted to some other service, which that
service can then replay against you. RFC 8707 makes the resource
