...
`AppAuthnModule::for_root(None)` reads `JwtConfig` from the environment;
...
secret: `AppAuthnGuard` returns a 401 without a valid
...
yields `AuthError::Expired`. `AppAuthnGuard` maps every variant to a 401
...
```rust title="crates/features/src/app_authn/claims.rs"
...
`AppAuthnGuard` records `actor_id` onto the request span, so every downstream
...
passed to `AppAuthnModule::for_root(config)`.
