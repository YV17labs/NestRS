...
`crates/features/src/app_oauth/` ships the full handler:
...
impl AppOAuthController {
    ...
    // NOT `#[public]`: `AppOAuthGuard` authenticates *from* `code`/`state`, so it
...
opposite — `code` and `state` *are* the credential, and `AppOAuthGuard` (your app's
`Strategy<Principal = Caller>`) is what verifies them. Marking it `#[public]`
tells `AppAuthnGuard` to admit a rejected credential as anonymous, which turns a
