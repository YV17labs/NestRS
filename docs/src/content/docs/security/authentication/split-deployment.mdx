...
`crates/features::app_authn` crate. The verifier deserializes a token
...
The issuer imports the OAuth flow controller (`AppOAuthHttpModule`) and
...
`features::app_authn::AppAuthnModule` — the product's own, which wraps that one
