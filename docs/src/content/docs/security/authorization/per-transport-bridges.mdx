...
| `AuthzMcpModule` | `AppMcpGuard` (`McpAbilityBridge<AuthGuard, AuthzGuard>`) as `dyn McpOperationGuard` | Re-runs the HTTP guard chain on the MCP endpoint, installs ambient ability for the tool call |
...
`AuthGuard` + `AuthzGuard` run at the upgrade. The upgrade's
...
(`McpAbilityBridge<AuthGuard, AuthzGuard>`) as `dyn McpOperationGuard` —
