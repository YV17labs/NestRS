...
description: AuthzHttpModule, AuthzGraphqlModule, and AuthzWsModule re-establish the ambient ability at each transport's dispatch point; MCP wires the McpAbilityBridge directly.
sidebar:
  order: 6
---
...
`dyn McpOperationGuard` (see [MCP](#mcp--same-shape-as-http) below).
