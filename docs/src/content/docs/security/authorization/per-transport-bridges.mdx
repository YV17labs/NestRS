...
| `AuthzMcpModule` | `AppMcpGuard` (`McpAbilityBridge<AuthnGuard, AuthzGuard>`) as `dyn McpOperationGuard` | Re-runs the HTTP guard chain on the MCP endpoint, installs ambient ability for the tool call |
...
(`McpAbilityBridge<AuthnGuard, AuthzGuard>`) as `dyn McpOperationGuard` —
...
guard's denial. Import `AuthzMcpModule` beside the feature module, the
same shape as the other transports.
