...
is called by `Repo::scoped(...)` only when the ability is present.
...
- Worker jobs and cron ticks — system work, no caller, no filter.
  Correct.
- An HTTP handler whose controller imported neither `AuthzHttpModule`
  nor a transport-specific authz module — the request runs without
  an ability, and `Repo::scoped(...)` returns every row. **Incorrect**
  — match every new controller with its authz module, and write a
