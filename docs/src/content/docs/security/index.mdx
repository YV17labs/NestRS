...
#[use_guards(AuthGuard, AuthzGuard)]   // ← bind both layers
...
$ TOKEN=$(curl -sX POST http://localhost:3001/token \
...
$ curl -s -o /dev/null -w "%{http_code}\n" \
    http://localhost:3000/users/0193c1ee-...other-org \
...
`apps/platform-auth` issues tokens (signs with the **private** EdDSA key);
`apps/platform-api` verifies them (holds **only** the public key). They share
...
```bash
just dev auth     # token issuer on :3001
just dev api      # resource server on :3000
```
...
- `crates/nest-rs-authn/` — the trait, the strategies, JWT signing/verifying.
- `crates/nest-rs-authz/` — `Ability`, `mask`, `condition_for`, the shaper.
