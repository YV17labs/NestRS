...
drives the graphql-ws protocol against the app's composed schema —
`connection_init` → `connection_ack` → `subscribe` → `next` — with no
socket bound:
...
upgrade itself: the guard that authenticates it, and the socket-lifetime
ceiling that bounds it. Those need a real connection, so a suite that must
cover them binds a transport (`build_headless` + `spawn_transport`) and
