...
`override_value` and `override_dyn` swap any provider after the
four-phase build. The override applies before any consumer resolves the
type, so controllers, resolvers, and guards pick it up.
