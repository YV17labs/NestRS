...
`TestApp::builder().module::<PublishApiModule>()` runs the same
...
```rust title="apps/publish-api/tests/e2e/users.rs"
...
- `just test-e2e` runs the whole suite from the workspace root.
