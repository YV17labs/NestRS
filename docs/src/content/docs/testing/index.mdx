...
async fn create_then_list_users() -> anyhow::Result<()> {
    let app = TestApp::<platform_api::PlatformApiModule>::builder()
...
- **`TestApp::<PlatformApiModule>::builder()`** runs the same boot path as `main`
...
async fn current_weather_returns_stubbed_report() -> anyhow::Result<()> {
    let app = TestApp::<mcp::McpModule>::builder()
...
|----------------------------------------------------|-----------------------------------------|--------|
| `apps/<app>/tests/e2e.rs`                          | The app's real top-level module end to end | ✓ real |
...
- **Every app ships a `tests/e2e.rs`** booting its real top-level module. It
...
- `crates/nestrs-testing/` — `TestApp`, `TestAppBuilder`, the in-process
  HTTP client.
- `apps/platform-api/tests/e2e.rs` and `apps/platform-auth/tests/e2e.rs` — the live
  baseline tests.
