...
Shared helpers live at the **suite root** — in `main.rs` itself, reached
...
```rust title="crates/nest-rs-authn/tests/integration/main.rs (abridged)"
...
```rust title="crates/nest-rs-authn/tests/integration/config.rs"
