...
stranger sees the public subset, and `#[expose(skip)]` columns never
escape.
...
mod tests {
    ...
    #[test]
    fn unrestricted_grant_still_strips_expose_skip_columns() {
        let masked = AppAbility::admin().mask(fixture()).expect("masked");
        assert!(
            masked.password_hash.is_empty(),
            "password_hash is #[expose(skip)] — must never leak even with full field grant",
        );
    }
}
```

The third test matters most. An admin gets every granted field, but a
`#[expose(skip)]` column (a password hash, an API secret) must still
be stripped on its way to the wire. A bug in `retain_wire_keys` leaks
it. The test pins the contract.
...
- **E2E negative path** — the wrong caller gets `403`, the masked
  response excludes `#[expose(skip)]` columns, a missing authz module
