...
`Ability::mask`, `Ability::condition_for`, and `WireModelDefaults` are
the policy code of a nestrs app — they decide which fields a caller
sees and which rows the data layer accepts. A regression there is a
silent data leak; a happy-path e2e does not catch it.

These are perfect unit-test targets: pure functions on policy types,
no DI, no DB, microsecond runs.
...
```rust title="crates/features/src/users/core/policy.rs"
#[cfg(test)]
mod tests {
    use super::*;
    use uuid::Uuid;
