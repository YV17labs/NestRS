...
1. Clone the reference repo and open it in the devcontainer — VS Code
   ...
   beside the workspace and installs the `nestrs` CLI. Then generate
   the JWT signing key the `auth` issuer refuses to boot without:

   ```bash frame="terminal"
   $ git clone https://github.com/NestRS/NestRS && code NestRS
   # accept "Reopen in Container", then in the container terminal:
   $ cd demo
   $ printf 'NESTRS_AUTHN__PRIVATE_KEY="%s"\n' \
       "$(openssl genpkey -algorithm ed25519 | sed ':a;N;$!ba;s/\n/\\n/g')" >> .env.local
   ```
   ...
   `NESTRS_QUEUE__URL` at `localhost` in `demo/.env.local`.
