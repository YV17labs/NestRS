...
```rust title="apps/tutorial-api/src/module.rs" {3-4, 14-15}
use nest_rs_core::module;
...
```bash frame="terminal"
$ NESTRS_GRAPHQL__EMIT_SDL=1 cargo run -p tutorial-api
INFO nest_rs::graphql: wrote schema to apps/tutorial-api/schema.graphql

$ head -5 apps/tutorial-api/schema.graphql
type Query {
  user(id: String!): User
