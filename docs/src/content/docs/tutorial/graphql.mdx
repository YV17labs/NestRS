...
<Aside type="caution">
This page applies to **`publish-api`**, not **`blog`**. The tutorial app
...
```rust title="apps/publish-api/src/module.rs" {14-15}
        UsersGraphqlModule,
...
```bash frame="terminal"
$ NESTRS_GRAPHQL__EMIT_SDL=1 just dev publish-api
INFO nest_rs::graphql: wrote schema to apps/publish-api/schema.graphql

$ head -5 apps/publish-api/schema.graphql
type Query {
  user(id: String!): User
