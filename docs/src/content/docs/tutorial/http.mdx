...
A DB-backed controller **needs** the two guards, and that is a property of the
data layer rather than a policy choice. `Repo` filters every read by the
caller's ambient [`Ability`](/security/authorization/); nothing but `AuthzGuard`
...
That writes `crates/features/src/{identity,authn,authz}/`, adds the two modules
