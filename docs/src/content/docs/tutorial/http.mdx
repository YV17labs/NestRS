...
every read and every by-id write; nothing but `AppAuthzGuard` installs one, so a
...
`AppAuthnGuard` and `AppAuthzGuard` are **your** types, not framework ones: the
...
```rust title="crates/features/src/app_authz/ability.rs" {2, 8}
...
(`PostsModule`) and `AppAuthzHttpModule`, which is what makes the ability guard
...
`PostsHttpModule` already imports `AppAuthzHttpModule` transitively, so listing
...
WARN nest_rs::layers: guard denied the request guard="…AppAuthnGuard<…>" status=401 trace_id=01a0156ebd7d77e0b78201a531dd9536 span_id=e0312d68a4872409
```
...
ERROR nest_rs::authz: missing request Ability — route is authorized but no ability guard ran
                      ...
                      hint="bind the ability guard (#[use_guards(AppAuthnGuard, AppAuthzGuard)]) …"
```
...
- A `PostsController` with the full CRUD surface `#[crud]` generated,
  behind `AppAuthnGuard` + `AppAuthzGuard`.
