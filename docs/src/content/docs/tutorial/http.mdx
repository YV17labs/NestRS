...
same post from `GET /posts/:id` — once Postgres is wired on the next page,
[Persist through Postgres](/tutorial/database/).
...
The `500` is honest — the route exists and ran; there is just no pool to
