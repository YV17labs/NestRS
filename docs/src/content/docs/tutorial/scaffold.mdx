...
description: Create apps/blog/ with nestrs new blog, start it with just dev blog, see a 404 on / — then read what the CLI generated.
sidebar:
  order: 2
---
...
page, `just dev blog` listens on the port pinned in `module.rs` and replies
...
[reference root](https://github.com/NestRS/NestRS/blob/main/apps/publish-api/src/module.rs)
...
<Aside type="tip">
`just dev blog` runs under `bacon` and restarts on every save.
</Aside>
