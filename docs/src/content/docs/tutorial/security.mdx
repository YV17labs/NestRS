...
`users` feature — your `blog` app from the previous pages keeps its
public CRUD. You add two guards to the controller — one for *who* the caller is, one
...
pub type AuthnGuard = nest_rs_authn::AuthnGuard<JwtStrategy<Claims>>;
```
...
Returning `403` (not `404`) when the row exists but the ability refuses
it does reveal that the row exists — nestrs accepts that leak
deliberately; see [by-id
binding](/security/authorization/by-id-binding/) for when to collapse
the `403` path into a `404`. A `500` would mean the ambient ability
