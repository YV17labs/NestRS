...
description: A single tests/e2e.rs boots the real BlogModule against a throwaway database; just test-e2e runs it; happy path and failure modes both asserted.
sidebar:
  order: 9
---
...
```rust title="apps/blog/tests/e2e.rs"
...
[`publish-api`](/testing/negative-path/) once auth and row-level policy
...
- A `tests/e2e.rs` booting the real `BlogModule` against a
...
- A `just test-e2e` recipe running them green.
...
matrix, open [`publish-api`](/publish/) and
