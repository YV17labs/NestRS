...
`Valid<Json<CreatePostInput>>`, so every request body is checked against
...
async fn create(
    &self,
    __body: ::nest_rs_http::Valid<::poem::web::Json<CreatePostInput>>,
...
`Valid<Json<CreatePostInput>>`. The
[`users` controller in `publish-api`](/tutorial/security/) is the
reference override.
...
`CreatePostInput` and `UpdatePostInput`. No second declaration on the
