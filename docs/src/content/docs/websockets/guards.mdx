...
**A guard on the `#[gateway]` struct needs no marker**: the upgrade is an
HTTP `GET`, so those are HTTP guards and `check_http` is what runs. The
bound is where that distinction stops being folklore.
</Aside>
