...
```json
{
  ...
  "data": {
    ...
    "errors": { "text": [{ "code": "length", "message": "text must not be empty" }] }
  }
}
```

`errors` is **absent**, not `null`, when the failure had nothing structured to
say, so a client can branch on its presence. A `warn!` lands in the `nest_rs::ws` target alongside the
...
<Aside type="caution">
Keep `Display` for the error variant wire-safe: the frame carries
`e.to_string()`, so avoid `#[error(transparent)]` over an ORM or `sqlx`
error whose message can echo a query. The frame is the one place a typed
error's `Display` becomes a client-visible string.
</Aside>
