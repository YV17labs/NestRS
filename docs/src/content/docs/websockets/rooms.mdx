...
The `chat` gateway uses all four scopes — joining at connect,
moving rooms by event, broadcasting peer-visible events, replying with
a payload:

```rust title="crates/features/src/chat/ws/gateway.rs (from the demo)"
#[gateway(path = "/ws")]
...
- [`WsClient`](https://github.com/YV17labs/NestRS/blob/main/crates/nest-rs-ws/src/server.rs) —
