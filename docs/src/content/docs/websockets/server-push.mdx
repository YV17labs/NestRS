...
```rust title="crates/features/src/notify/schedule/tasks.rs"
use std::sync::Arc;
...
and broadcasts the derived event. `crates/features/src/chat/service.rs`
shows exactly this:

```rust title="crates/features/src/chat/service.rs"
#[injectable]
...
<Aside type="note">
"Unbounded" here means the in-process channel is unbounded. Backpressure
policy is yours to set, on purpose: the right answer differs per traffic
shape — disconnect on outbox depth, drop on TTL, coalesce successive
frames. The registry hands back the accepted-frame count, and
`WsServer::connection_count` gives you the other half of the signal.
</Aside>

## Reference

- [`WsServer`](https://github.com/YV17labs/NestRS/blob/main/crates/nest-rs-ws/src/server.rs) —
  `broadcast`, `emit_to`, `emit`, `connection_count`.
- [`crates/features/src/chat/service.rs`](https://github.com/YV17labs/NestRS/blob/main/demo/crates/features/src/chat/service.rs) —
